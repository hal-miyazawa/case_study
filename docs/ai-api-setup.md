...
現在のリポジトリには、OpenAI API を試すための開発用エンドポイント `/api/ai-scene` が残っています。
ただし、現在実装中の本編フローでは、予告本画面から対策フェーズ、リュック画面までは AI を使わない固定進行にしています。

AI は、今後ショップで買ったものや選択した対策をもとに、次の文章や選択肢を生成する部分で使う想定です。

API キーがある場合とない場合で動きが変わります。

| 状態 | 動き |
| --- | --- |
| `OPENAI_API_KEY` あり | `/api/ai-scene` を直接テストしたときに OpenAI API に通信する |
| `OPENAI_API_KEY` なし | `/api/ai-scene` を直接テストしたときに固定のモック文章を返す |

そのため、API キーを持っていない人でも現在の画面や UI の確認はできます。
...
現時点では、本編画面上から OpenAI API を呼ぶ導線はまだ固定していません。
API キーありで確認したい場合は、下の「直接 API をテストする方法」を使います。
...
```text
API キーなしの人:
UI、CSS、ショップ画面、固定データ、ドキュメント、通常の画面遷移を確認する
